...
from baudot.handlers import TapeConfig, TapeWriter, TapeReader
...
def tape_config_strategy(draw):
    ...
    return TapeConfig(punch, blank, sep)
...
def test_tape_codec_tnb(codec_test, tape_config):
    ...
    tmp_out = StringIO()
    writer = TapeWriter(tmp_out, tape_config)
    encode_str(test_str, codec, writer)

    tmp_out.seek(0)
    reader = TapeReader(tmp_out, tape_config)
